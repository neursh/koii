...
fn worker(mut rx: mpsc::Receiver<(VerifyEmailRequest, Option<oneshot::Sender<Option<()>>>)>) {
  ...
  let resend_token = std::env::var("RESEND_TOKEN").expect("RESEND_TOKEN must be set in .env file");
...
fn create_verify_base(request: &VerifyEmailRequest) -> CreateEmailBaseOptions {
  ...
  variables.insert(
    ...
    serde_json::Value::String(
      format!("https://koii.neurs.space/verify?token={}", request.verify_token)
    )
  );

  CreateEmailBaseOptions::new(
    "Koii <koii@verify.neurs.space>",
    [&request.email],
